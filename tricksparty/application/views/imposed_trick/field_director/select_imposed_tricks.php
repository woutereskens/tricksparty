...
<?php echo form_open('Trick/selectImposedTricks', array('role' => 'form')); ?>
<div class="row">
    <div class="col-md-6">
        <h3 class="text-center"><?php echo $this->lang->line("imposed_tricks"); ?></h3>
        <table class="table">
            <col width="30%">
            <col width="70%">
            <tr>
            ...
            <?php foreach ($imposed_trick_types as $imposed_trick_type) { ?>
                <tr>
                    <td class="text-center">
                        <label for="<?php echo substr($imposed_trick_type->ScoreType, 0, 3); ?>"><?php echo substr($imposed_trick_type->ScoreType, 2, 1); ?></label>
                    </td>
                    <td class="text-center">
                        <select name="<?php echo substr($imposed_trick_type->ScoreType, 0, 3); ?>" id="<?php echo substr($imposed_trick_type->ScoreType, 0, 3); ?>" class="js-example-basic-single form-control"  style="width: 100%">
                            <?php foreach ($trickgroups as $trickgroup) { ?>
                                <optgroup label="<?php echo $this->lang->line("group") . ' ' . $trickgroup->TrickGroup; ?>">
                                    <?php foreach ($trickgroup->Tricks as $trick) { ?>
                                        <option value="<?php echo $trick->Trick; ?>" <?php echo set_select(substr($imposed_trick_type->ScoreType, 0, 3), $trick->Trick); ?>><?php echo $trick->Trick; ?></option>
                                    <?php } ?>
                                </optgroup>
                            <?php } ?>
                        </select>
